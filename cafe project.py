...
# 1.Добавить минимум 10 блюд.
# 2.Добавить минимум 4 категории.
# 3.Сделать поиск блюда по названию.
# 4.Сделать фильтр по категории.
# 5.Сделать удаление блюда из заказа.
# 6.Сделать изменение количества.
# 7.Добавить скидку от 10000 тг.
# 8.Добавить историю заказов.
# 9.Сделать проверку, что нельзя заказать больше 10 одинаковых блюд.
...
def search_by_name(name):
    """Поиск блюда по названию (пункт 67)."""
    return [d for d in menu if name.lower() in d["name"].lower()]


def filter_by_category(category):
    """Фильтр по категории (пункт 68)."""
...
def add_to_order(dish_id, quantity=1):
    ...
    for item in order:
        if item["dish"]["id"] == dish_id:
            # Проверка: нельзя больше 10 (пункт 73)
...
def remove_from_order(dish_id):
    """Удаление блюда из заказа (пункт 69)."""
    for item in order:
        if item["dish"]["id"] == dish_id:
            order.remove(item)
...
def place_order():
    """Оформить заказ и добавить в историю (пункт 72)."""
    if not order:
        print("Заказ пуст.")
        return
